...
const AUTH_STATE_COOKIE = "playrush_oauth_state";
...
async function initDb() {
  ...
  await db.query(`
    ...
    CREATE TABLE IF NOT EXISTS auth_identities (
      ...
      provider text NOT NULL, subject text NOT NULL, password_hash text,
      created_at timestamptz NOT NULL DEFAULT now(), last_login_at timestamptz,
      UNIQUE(provider, subject)
    );
    CREATE UNIQUE INDEX IF NOT EXISTS auth_password_email_idx ON auth_identities(lower(subject)) WHERE provider='password';
...
function authLimited(req, bucket, max=12, windowMs=10*60_000) {
  const key=`${bucket}:${req.ip||req.socket?.remoteAddress||'unknown'}`; const now=Date.now();
  const item=authRate.get(key); if(!item || now-item.start>windowMs){authRate.set(key,{start:now,count:1});return false;}
  item.count++; return item.count>max;
}
function setOAuthState(res, provider) {
  const nonce=crypto.randomBytes(24).toString('base64url');
  const payload=JSON.stringify({provider,nonce,exp:Date.now()+AUTH_STATE_TTL_MS});
  const encoded=Buffer.from(payload).toString('base64url');
  const sig=sign(encoded);
  res.setHeader('Set-Cookie',`${AUTH_STATE_COOKIE}=${encodeURIComponent(`${encoded}.${sig}`)}; Path=/; HttpOnly; SameSite=${process.env.NODE_ENV==='production'?'None; Secure':'Lax'}; Max-Age=600`);
  return {nonce,state:`${encoded}.${sig}`};
}
function consumeOAuthState(req,res,provider) {
  const raw=parseCookies(req)[AUTH_STATE_COOKIE];
  res.setHeader('Set-Cookie',`${AUTH_STATE_COOKIE}=; Path=/; HttpOnly; SameSite=${process.env.NODE_ENV==='production'?'None; Secure':'Lax'}; Max-Age=0`);
  if(!raw)return null; const [encoded,sig]=raw.split('.'); if(!encoded||!sig)return null;
  const expected=sign(encoded); if(sig.length!==expected.length || !crypto.timingSafeEqual(Buffer.from(sig),Buffer.from(expected)))return null;
  try { const p=JSON.parse(Buffer.from(encoded,'base64url').toString()); if(p.provider!==provider||p.exp<Date.now())return null; return p; } catch{return null;}
}
function redirectUri(provider){return `${PUBLIC_URL}/api/auth/${provider}/callback`;}
function safeHandle(seed='fan') { const base=String(seed||'fan').toLowerCase().replace(/[^a-z0-9_]/g,'').slice(0,15)||'fan'; return `${base}_${crypto.randomBytes(3).toString('hex')}`.slice(0,24); }
async function uniqueHandle(seed){
  for(let i=0;i<8;i++){const h=safeHandle(i?`${seed}${i}`:seed); if(db){if(!(await db.query('SELECT 1 FROM users WHERE lower(handle)=lower($1)',[h])).rowCount)return h;}else if(![...memory.users.values()].some(u=>u.handle.toLowerCase()===h.toLowerCase()))return h;}
  return `fan_${crypto.randomBytes(5).toString('hex')}`.slice(0,24);
}
async function findIdentity(provider, subject){
  if(db)return (await db.query('SELECT ai.user_id AS "userId", u.email, u.handle FROM auth_identities ai JOIN users u ON u.id=ai.user_id WHERE ai.provider=$1 AND ai.subject=$2',[provider,subject])).rows[0]||null;
  return [...memory.authIdentities.values()].find(x=>x.provider===provider&&x.subject===subject)||null;
}
async function findUserByEmail(email){
  const normalized=String(email||'').trim().toLowerCase(); if(!normalized)return null;
  if(db)return (await db.query('SELECT id,handle,email,coins,xp FROM users WHERE lower(email)=lower($1) LIMIT 1',[normalized])).rows[0]||null;
  return [...memory.users.values()].find(u=>String(u.email||'').toLowerCase()===normalized)||null;
}
async function createIdentity(userId,provider,subject,passwordHash=null){
  if(db){await db.query('INSERT INTO auth_identities(id,user_id,provider,subject,password_hash,last_login_at) VALUES($1,$2,$3,$4,$5,now()) ON CONFLICT(provider,subject) DO UPDATE SET last_login_at=now()',[uid(),userId,provider,subject,passwordHash]);}
  else {if(!memory.authIdentities)memory.authIdentities=new Map();const existing=[...memory.authIdentities.values()].find(x=>x.provider===provider&&x.subject===subject);if(existing){existing.lastLoginAt=nowIso();return;}memory.authIdentities.set(uid(),{userId,provider,subject,passwordHash,lastLoginAt:nowIso()});}
}
async function getPasswordIdentity(email){
  const subject=String(email||'').trim().toLowerCase(); if(!subject)return null;
  if(db)return (await db.query('SELECT ai.user_id AS "userId",ai.password_hash AS "passwordHash",u.email,u.handle,u.coins,u.xp FROM auth_identities ai JOIN users u ON u.id=ai.user_id WHERE ai.provider=\'password\' AND lower(ai.subject)=lower($1)',[subject])).rows[0]||null;
  if(!memory.authIdentities)return null; const x=[...memory.authIdentities.values()].find(v=>v.provider==='password'&&v.subject===subject); return x?{...x,userId:x.userId,passwordHash:x.passwordHash,...memory.users.get(x.userId)}:null;
}
async function createPermanentUser({email,name,provider,subject,passwordHash=null}){
  const normalized=String(email||'').trim().toLowerCase()||null;
  const existingIdentity=await findIdentity(provider,subject); if(existingIdentity){const user=await getUser(existingIdentity.userId); if(user){if(normalized&&user.email!==normalized)await updateUser(user.id,{email:normalized}); await createIdentity(user.id,provider,subject,passwordHash); return user;}}
  let user=normalized?await findUserByEmail(normalized):null;
  if(!user){const id=uid();const handle=await uniqueHandle(name||'fan');user={id,handle,email:normalized,coins:1000,xp:0};if(db){await db.query('INSERT INTO users(id,handle,email,coins,xp) VALUES($1,$2,$3,$4,$5)',[id,handle,normalized,1000,0]);await db.query('INSERT INTO wallet_transactions(id,user_id,kind,amount,balance_after,description) VALUES($1,$2,\'starter\',1000,1000,\'Starter Rush Coins\')',[uid()]);}else{memory.users.set(id,user);walletTransactionMemory(id,'starter',1000,1000,null,'Starter Rush Coins');}}
  await createIdentity(user.id,provider,subject,passwordHash); return user;
}
function authCookie(res,userId){setSession(res,userId);}
function authRedirect(res,success=true,message=''){const q=success?'auth=success':'auth=error';res.redirect(`${PUBLIC_URL}/#home?${q}${message?`&message=${encodeURIComponent(message)}`:''}`);}
...
async function exchangeCode(tokenUrl,params){const r=await fetch(tokenUrl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams(params)});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error_description||data.error||`OAuth token exchange failed (${r.status})`);return data;}
async function appleClientSecret(){
  if(!APPLE_TEAM_ID||!APPLE_KEY_ID||!APPLE_PRIVATE_KEY||!APPLE_CLIENT_ID)throw new Error('Apple Sign in is not configured.');
  const key=await importPKCS8(APPLE_PRIVATE_KEY.replace(/\\n/g,'\n'),'ES256');
  return new SignJWT({}).setProtectedHeader({alg:'ES256',kid:APPLE_KEY_ID}).setIssuer(APPLE_TEAM_ID).setSubject(APPLE_CLIENT_ID).setAudience('https://appleid.apple.com').setIssuedAt().setExpirationTime('180d').sign(key);
}
async function verifyGoogle(idToken){const {payload}=await jwtVerify(idToken,GOOGLE_JWKS,{issuer:['https://accounts.google.com','accounts.google.com'],audience:GOOGLE_CLIENT_ID});return payload;}
async function verifyApple(idToken){const {payload}=await jwtVerify(idToken,APPLE_JWKS,{issuer:'https://appleid.apple.com',audience:APPLE_CLIENT_ID});return payload;}
...
app.use((req,res,next)=>{res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Referrer-Policy','strict-origin-when-cross-origin');res.setHeader('X-Frame-Options','SAMEORIGIN');if(process.env.NODE_ENV==='production')res.setHeader('Strict-Transport-Security','max-age=31536000; includeSubDomains');next();});
...
app.get('/api/auth/providers',(_req,res)=>res.json({email:true,google:Boolean(GOOGLE_CLIENT_ID&&GOOGLE_CLIENT_SECRET),apple:Boolean(APPLE_CLIENT_ID&&APPLE_TEAM_ID&&APPLE_KEY_ID&&APPLE_PRIVATE_KEY),redirects:{google:redirectUri('google'),apple:redirectUri('apple')}}));
app.get('/api/auth/status',async(req,res)=>{const user=await getUser(validSession(req));res.json({authenticated:Boolean(user),user:user||null});});
app.post('/api/auth/register',async(req,res)=>{try{if(authLimited(req,'register'))return res.status(429).json({error:'Too many sign-up attempts. Try again shortly.'});const email=String(req.body?.email||'').trim().toLowerCase(),password=String(req.body?.password||''),name=String(req.body?.name||'').trim().slice(0,60);if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return res.status(400).json({error:'Enter a valid email address.'});if(password.length<8||password.length>128)return res.status(400).json({error:'Password must be 8–128 characters.'});if(await findIdentity('password',email))return res.status(409).json({error:'An account already exists with that email.'});const hash=await bcrypt.hash(password,12);const user=await createPermanentUser({email,name:name||email.split('@')[0],provider:'password',subject:email,passwordHash:hash});authCookie(res,user.id);res.status(201).json({ok:true,user});}catch(e){res.status(400).json({error:e.message||'Unable to create account.'});}});
app.post('/api/auth/login',async(req,res)=>{try{if(authLimited(req,'login'))return res.status(429).json({error:'Too many login attempts. Try again shortly.'});const email=String(req.body?.email||'').trim().toLowerCase(),password=String(req.body?.password||'');const identity=await getPasswordIdentity(email);if(!identity||!identity.passwordHash||!(await bcrypt.compare(password,identity.passwordHash)))return res.status(401).json({error:'Email or password is incorrect.'});await createIdentity(identity.userId,'password',email,identity.passwordHash);authCookie(res,identity.userId);res.json({ok:true,user:await getUser(identity.userId)});}catch(e){res.status(400).json({error:e.message||'Unable to sign in.'});}});
app.post('/api/auth/logout',(req,res)=>{clearSession(res);res.json({ok:true});});
app.get('/api/auth/google', (req,res)=>{if(!GOOGLE_CLIENT_ID||!GOOGLE_CLIENT_SECRET)return res.status(503).send('Google Sign in is not configured.');const {nonce,state}=setOAuthState(res,'google');const u=new URL('https://accounts.google.com/o/oauth2/v2/auth');u.searchParams.set('client_id',GOOGLE_CLIENT_ID);u.searchParams.set('redirect_uri',redirectUri('google'));u.searchParams.set('response_type','code');u.searchParams.set('scope','openid email profile');u.searchParams.set('state',state);u.searchParams.set('nonce',nonce);res.redirect(u.toString());});
app.get('/api/auth/google/callback',async(req,res)=>{try{const st=consumeOAuthState(req,res,'google');if(!st||!req.query.code)throw new Error('Google sign-in session expired. Please try again.');const tokens=await exchangeCode('https://oauth2.googleapis.com/token',{code:String(req.query.code),client_id:GOOGLE_CLIENT_ID,client_secret:GOOGLE_CLIENT_SECRET,redirect_uri:redirectUri('google'),grant_type:'authorization_code'});const payload=await verifyGoogle(tokens.id_token);if(payload.nonce!==st.nonce)throw new Error('Google sign-in verification failed.');const user=await createPermanentUser({email:payload.email,name:payload.name||payload.email?.split('@')[0],provider:'google',subject:payload.sub});authCookie(res,user.id);authRedirect(res); }catch(e){authRedirect(res,false,e.message||'Google sign-in failed.');}});
app.get('/api/auth/apple',async(req,res)=>{try{if(!APPLE_CLIENT_ID||!APPLE_TEAM_ID||!APPLE_KEY_ID||!APPLE_PRIVATE_KEY)return res.status(503).send('Apple Sign in is not configured.');const {nonce,state}=setOAuthState(res,'apple');const u=new URL('https://appleid.apple.com/auth/authorize');u.searchParams.set('client_id',APPLE_CLIENT_ID);u.searchParams.set('redirect_uri',redirectUri('apple'));u.searchParams.set('response_type','code');u.searchParams.set('response_mode','form_post');u.searchParams.set('scope','name email');u.searchParams.set('state',state);u.searchParams.set('nonce',nonce);res.redirect(u.toString());}catch(e){res.status(500).send(e.message);}});
app.post('/api/auth/apple/callback',async(req,res)=>{try{const st=consumeOAuthState(req,res,'apple');if(!st||!req.body?.code)throw new Error('Apple sign-in session expired. Please try again.');const clientSecret=await appleClientSecret();const tokens=await exchangeCode('https://appleid.apple.com/auth/token',{code:String(req.body.code),client_id:APPLE_CLIENT_ID,client_secret:clientSecret,redirect_uri:redirectUri('apple'),grant_type:'authorization_code'});const payload=await verifyApple(tokens.id_token);if(payload.nonce!==st.nonce)throw new Error('Apple sign-in verification failed.');let email=payload.email||null,name='Apple fan';if(req.body.user){try{const info=JSON.parse(req.body.user);name=[info?.name?.firstName,info?.name?.lastName].filter(Boolean).join(' ')||name;}catch{}}const user=await createPermanentUser({email,name,provider:'apple',subject:payload.sub});authCookie(res,user.id);authRedirect(res);}catch(e){authRedirect(res,false,e.message||'Apple sign-in failed.');}});
...
if(process.env.NODE_ENV==='production' && (!process.env.SESSION_SECRET || process.env.SESSION_SECRET==='dev-only-change-me')){
  console.error('SESSION_SECRET must be configured in production.'); process.exit(1);
}
